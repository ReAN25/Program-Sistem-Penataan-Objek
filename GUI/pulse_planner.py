...
class PulsePlan:
    ...
    slice_us: int
    block_ms: int
    # Block yang sudah dikonfirmasi boleh dikosongkan oleh transport. Slot
    # list tetap dipertahankan agar jumlah total/sequence tetap stabil dan
    # blok dapat dibangun ulang saat firmware meminta sequence lama.
    ...
    def get_block(self, sequence: int) -> PulseBlock:
        """Ambil blok berdasarkan sequence, membangunnya ulang bila dicache kosong."""
    ...
    def release_block(self, sequence: int) -> None:
        """Lepaskan objek blok yang sudah selesai untuk menghemat RAM."""
        sequence = int(sequence)
...
def _make_plan(
    ...
    # Z tetap menjadi metadata grafik/CSV dan tidak menghasilkan pulsa.
    ...
    # z_mm × (deg/cm ÷ 10) × 10 deci-deg/deg = z_mm × deg/cm.
...
def plan_polyline(
    ...
    delta_xyz = np.diff(points, axis=0)
    # Input kecepatan tidak dibatasi. Jika kebutuhan pulsa melebihi satu pulsa
    # per tick ISR, waktu otomatis diperpanjang agar paket tetap dapat
    # dieksekusi tanpa NAK:RATE atau kehilangan langkah.
    ...
    # Generator Z firmware memakai interval tick integer. Contoh 20 mm/s pada
    # konfigurasi 15 deg/cm menjadi 5 tick atau 18,75 mm/s aktual. Gunakan nilai
    # diskrit yang benar agar timeline tidak mendahului kemampuan fisik Z.
    ...
    # Metrik waktu tiap segmen ditentukan sumbu yang paling lama. Dengan ini
    # X dan Y tetap selesai bersamaan, tetapi masing-masing memakai batas
